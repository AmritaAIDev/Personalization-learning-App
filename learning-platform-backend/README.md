...
6. `npm run seed:demo-experience` — creates/enriches the demo learner with history, practice mistakes, doubts, and multi-subject progress.
7. `npm run start:dev` — API on `http://localhost:4000` (override with `PORT`).
...
Demo account for seeded environments:

- Email: `demo.student@jeeai.local`
- Password: `Demo@12345`
- Run: `npm run seed:demo-experience`
- Override with `DEMO_USER_EMAIL`, `DEMO_USER_PASSWORD`, `DEMO_USER_NAME`.
- Set `RESET_DEMO_PASSWORD=false` if an existing demo user's password should not be reset.

RAG/vector grounding:

- Run `npm run seed:qdrant:full` after setting `QDRANT_URL` and `QDRANT_API_KEY`.
- The seed upserts reviewed concept chunks into the `learning_concepts` collection for the same topics/subtopics used by practice, doubts, and adaptive tutoring.
