...
`flashcards` and `flashcard_reviews` implement database-backed spaced recall. `tutor_conversations` and `tutor_messages` preserve the side-assistant thread for a session.
...
- `GET /api/learning/flashcards`, `POST /api/learning/flashcards/:id/review` — recall deck and SRS rating.
