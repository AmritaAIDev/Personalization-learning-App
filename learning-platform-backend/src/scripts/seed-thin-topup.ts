...
const SEEDS: Seed[] = [
  ...
  q('TOPUP-EFL-M1', ECF, 'Electric Field Lines',
    'Two equal positive charges are placed at a small separation. Near the midpoint between them, the electric field lines:',
    ['Both point toward each charge', 'Cancel to zero everywhere', 'Bend away from the midpoint, leaving a low-field region', 'Run straight from one to the other'],
    ...
    'Medium', 'Apply', ['field lines', 'superposition'],
    ['Assuming the field is zero at every point on the perpendicular bisector.']),
  q('TOPUP-EFL-H1', ECF, 'Electric Field Lines',
    'A closed loop of electrostatic field lines would imply that the electrostatic field is:',
    ['Conservative', 'Non-conservative, which contradicts electrostatics', 'Uniform', 'Zero everywhere'],
  ...
  q('TOPUP-CL-H1', ECF, "Coulomb's Law",
    ...
    'Hard', 'Apply', ["Coulomb's law", 'inverse-square'],
    ['Forgetting that halving the distance squares the factor.'], 5, 120),
  q('TOPUP-CL-H2', ECF, "Coulomb's Law",
    ...
    'Hard', 'Analyze', ["Coulomb's law", 'optimisation'],
    ['Assuming the force is maximised when one part is as large as possible.'], 5),

  // Electric Dipole (needs Hard)
  q('TOPUP-ED-H1', ECF, 'Electric Dipole',
    ...
    'Hard', 'Apply', ['dipole', 'torque'],
    ['Confusing torque with potential energy, which is minimum at 90 degrees.'], 5),
  q('TOPUP-ED-H2', ECF, 'Electric Dipole',
    'A dipole placed in a uniform electric field has:',
    ['Net force and net torque both zero', 'Net force zero, net torque generally nonzero', 'Net force nonzero, net torque zero', 'Both nonzero'],
    ...
    'Hard', 'Understand', ['dipole', 'uniform field'],
    ['Assuming a uniform field can exert a net force on a neutral dipole.'], 5),

  // Capacitance (needs Hard)
  q('TOPUP-CAP-H1', EPC, 'Capacitance',
    ...
    'Hard', 'Apply', ['capacitance', 'parallel plate'],
    ['Applying only one of the two changes.'], 5),
  q('TOPUP-CAP-H2', EPC, 'Capacitance',
    ...
    'Hard', 'Apply', ['series capacitors', 'equivalent capacitance'],
    ['Adding series capacitances like parallel ones.'], 5),

  // Electric Potential (needs Medium, Hard)
  q('TOPUP-EP-M1', EPC, 'Electric Potential',
    ...
    'Medium', 'Remember', ['potential', 'point charge'],
    ['Confusing potential (1/r) with field (1/r^2).'], 4),
  q('TOPUP-EP-H1', EPC, 'Electric Potential',
    ...
    'Hard', 'Apply', ['potential', 'work'],
    ['Reversing the sign convention for work by the field vs work by an external agent.'], 5),

  // Capacitance basics (needs Medium, Hard)
  q('TOPUP-CB-M1', EPC, 'Capacitance basics',
    'The SI unit of capacitance is:',
    ['Farad', 'Coulomb', 'Volt', 'Ohm'],
    'Farad',
    'Capacitance is measured in farads (F), where 1 F = 1 C/V.',
    'Medium', 'Remember', ['capacitance', 'units'],
    ['Mixing capacitance units with charge or voltage units.'], 4),
  q('TOPUP-CB-H1', EPC, 'Capacitance basics',
    ...
    'Hard', 'Apply', ['energy', 'capacitor'],
    ['Using QV (without the 1/2) as the stored energy.'], 5),

  // Energy Stored in a Capacitor (needs Hard)
  q('TOPUP-ESC-H1', EPC, 'Energy Stored in a Capacitor',
    'A 10 microF capacitor is charged to 100 V. The energy stored is:',
    ['0.05 J', '0.5 J', '5 J', '50 J'],
    '0.05 J',
    'U = C V^2 / 2 = 10e-6 * 100^2 / 2 = 0.05 J.',
    'Hard', 'Apply', ['energy', 'capacitor'],
    ['Forgetting the factor of 1/2 in U = CV^2/2.'], 5),
  q('TOPUP-ESC-H2', EPC, 'Energy Stored in a Capacitor',
    ...
    'Hard', 'Apply', ['energy', 'capacitor'],
    ['Assuming energy scales linearly with voltage.'], 5),

  // Equipotential Surfaces (needs Hard)
  q('TOPUP-ES-H1', EPC, 'Equipotential Surfaces',
    'The electric field is always:',
    ['parallel to equipotential surfaces', 'perpendicular to equipotential surfaces', 'tangent to equipotential surfaces', 'zero on equipotential surfaces'],
    ...
    'Hard', 'Understand', ['equipotential', 'electric field'],
    ['Believing the field is parallel to equipotential surfaces.'], 5),
  q('TOPUP-ES-H2', EPC, 'Equipotential Surfaces',
    ...
    'Hard', 'Apply', ['equipotential', 'work'],
    ['Assuming any separation implies a potential difference.'], 5),

  // Van de Graaff Generator (needs Hard)
  q('TOPUP-VG-H1', EPC, 'Van de Graaff Generator',
    'A Van de Graaff generator builds a high potential on its dome because:',
    ['charge is carried to the dome continuously by the belt', 'the dome stores magnetic energy', 'it converts AC to DC inside the dome', 'the belt acts as a resistor'],
    ...
    'Hard', 'Understand', ['Van de Graaff', 'electrostatics'],
    ['Confusing it with an electromagnetic induction device.'], 5),
  q('TOPUP-VG-H2', EPC, 'Van de Graaff Generator',
    'The maximum potential of a Van de Graaff dome is limited mainly by:',
    ['the belt speed', 'dielectric breakdown of the surrounding gas', 'the colour of the dome', 'the ground resistance'],
    ...
    'Hard', 'Apply', ['Van de Graaff', 'dielectric breakdown'],
    ['Assuming belt speed is the limiting factor.'], 5),

  // Electric charge & field (needs Medium, Hard)
  q('TOPUP-ECF-M1', ECF, 'Electric charge & field',
    ...
    ['maximum along a diagonal', 'zero', 'along a side', 'depends on the side length'],
    'zero',
    'By symmetry, the fields from the four equal corner charges cancel pairwise at the centre, giving zero net field.',
    'Medium', 'Apply', ['superposition', 'symmetry'],
    ['Ignoring the symmetry and thinking the fields add along a diagonal.'], 4),
  q('TOPUP-ECF-H1', ECF, 'Electric charge & field',
    ...
    'Hard', 'Apply', ['ring of charge', 'symmetry'],
    ['Treating the ring as a point charge at its centre.'], 5),
];
...
async function main() {
  ...
  for (const seed of SEEDS) {
    const existing = await repo.findOne({ where: { question_id: seed.question_id } });
  ...
  console.log(`Top-up seeder: processed ${SEEDS.length} questions (${inserted} new, ${SEEDS.length - inserted} already present).`);
