...
export class SessionsService {
  ...
  async getJourneyForUser(userId: string) {
    ...
    subtopics.forEach((sub) => {
      ...
      const requiresHistoryNode = sub.prerequisites?.some((p) =>
        historyTopicIds.has(p.id),
      );
    ...
    const journey = chapters
      .map((chapter) => {
        const chapterSubtopics = subtopics.filter(
          (sub) =>
            sub.parent?.id === chapter.id &&
            personalizedSubtopicIds.has(sub.id),
        );
        ...
        const mappedSubtopics = chapterSubtopics.map((sub) => {
          ...
          else if (sub.prerequisites?.some((p) => historyTopicIds.has(p.id)))
            nodeType = 'next-step';
        ...
        if (chapterSession) {
          state =
            chapterSession.status === 'completed' ? 'completed' : 'active';
