...
function rankTopics(topics: TopicCompetency[]) {
  return topics.slice().sort((a, b) => a.score - b.score).slice(0, 6);
}
...
export default function AnalyticsPage() {
  ...
  const weakTopics = useMemo(() => rankTopics(growth?.topics ?? []), [growth?.topics]);
      ...
      <main className="mx-auto w-full max-w-6xl px-5 pt-9 sm:px-8 lg:px-10">
        <header className="grid gap-5 lg:grid-cols-[1fr_0.42fr] lg:items-end">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-ink-mute">
              Topic readiness, weak areas, diagnostic history, and route momentum are pulled
              directly from the learning database.
            </p>
          </div>
          <Link
            href="/tests"
        ...
        {loading ? (
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {[0, 1, 2].map((item) => (
              <div key={item} className="h-32 rounded-[1.35rem] border border-hairline bg-white p-5">
                ...
                <div className="mt-5 space-y-5">
                  {subjectRows.length > 0 ? (
                    ...
                    <p className="rounded-2xl border border-dashed border-hairline bg-canvas p-5 text-sm leading-6 text-ink-soft">
                      Analytics will populate after you complete a diagnostic or learning session.
                    </p>
                  )}
                </div>
              </article>
