...
function DashboardSkeleton() {
  return (
    <div className="mt-8 space-y-7" aria-label="Loading dashboard">
      ...
      <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_18rem]"><div className="h-44 rounded-2xl skeleton" /><div className="h-44 rounded-2xl skeleton" /></div>
    </div>
  );
}
...
export default function DashboardPage() {
  ...
  const loadDashboard = useCallback(async () => {
    setLoading(true);
    try {
      const summary = await apiFetch<StudentDashboardPayload>('/api/dashboard/student');
      setData(summary);
      setError(null);
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : 'Your learning dashboard could not be loaded.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#fbfbfd] pb-20">
      <main className="mx-auto w-full max-w-6xl px-5 pt-9 sm:px-8 lg:px-10">
        ...
        {error ? (
          <div className="mt-6 flex items-start gap-3 rounded-2xl border border-rose-100 bg-rose-50 p-4 text-sm text-rose-800" role="alert">
            <CircleAlert className="mt-0.5 h-5 w-5 shrink-0" aria-hidden="true" />
            <div><p className="font-semibold">Something needs attention</p><p className="mt-1">{error}</p><button type="button" onClick={() => void loadDashboard()} className="mt-3 font-semibold underline underline-offset-4">Try again</button></div>
          </div>
        ) : null}
