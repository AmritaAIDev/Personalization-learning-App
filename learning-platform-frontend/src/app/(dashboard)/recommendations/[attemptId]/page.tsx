...
function ResourceCard({ resource }: { resource: LearningResource }) {
  ...
  return (
    <article className="flex h-full flex-col rounded-2xl border border-[#ececf0] bg-white p-5 shadow-[0_8px_22px_rgba(20, 20, 30,0.04)]">
      ...
      <h3 className="mt-4 font-heading text-lg font-bold text-[#1a1a1f]">
        {resource.title}
      </h3>
      ...
      {resource.content && (
        <p className="mt-4 rounded-xl bg-[#f4f4f6] p-3 text-sm leading-6 text-[#52525b]">
          {resource.content}
        </p>
      )}
      {url && (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="mt-auto inline-flex items-center gap-2 pt-5 text-sm font-bold text-[#3f6f57] hover:text-[#315844]"
        >
...
export default function RecommendationsPage() {
  ...
  if (loading)
    return (
      <div className="grid min-h-screen place-items-center text-sm font-semibold text-[#52525b]">
  ...
  if (!recommendations) {
    return (
      <div className="mx-auto flex min-h-screen max-w-xl flex-col items-center justify-center p-6 text-center">
        ...
        <h1 className="mt-4 font-heading text-2xl font-bold text-[#1a1a1f]">
        ...
        <button
          type="button"
          onClick={() => void load()}
          className="mt-5 rounded-xl bg-[#3f6f57] px-4 py-2.5 text-sm font-bold text-white hover:bg-[#315844]"
        >
          Try again
        </button>
      </div>
  ...
  return (
    <div className="mx-auto max-w-7xl p-5 sm:p-8 lg:p-10">
      <Link
        ...
        className="inline-flex items-center gap-2 text-sm font-bold text-[#52525b] hover:text-[#3f6f57]"
      >
      ...
      <header className="mt-7 rounded-[2rem] border border-[#f2d9df] bg-[#fffafa] p-6 sm:p-8">
        {recommendations.hasWeakTopics ? (
          <>
            ...
            <h1 className="mt-2 font-heading text-3xl font-bold tracking-tight text-[#1a1a1f]">
      ...
      <section className="mt-10">
        <div className="mb-5">
          <p className="text-sm font-bold text-[#3f6f57]">Revision library</p>
        ...
        {recommendations.generalResources.length > 0 ? (
          ...
          <p className="rounded-2xl border border-dashed border-[#dedee3] bg-white p-6 text-sm text-[#52525b]">
            <BookOpenText
              className="mr-2 inline h-5 w-5 text-[#3f6f57]"
              aria-hidden="true"
            />
