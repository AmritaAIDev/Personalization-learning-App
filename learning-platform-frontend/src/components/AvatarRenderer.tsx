  ...
  return (
    <section className="relative flex aspect-video w-full flex-col justify-between overflow-hidden rounded-2xl border border-slate-800 bg-slate-950 p-5 text-white shadow-xl">
      ...
      <div className="relative flex items-center justify-between">
        <span className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.16em] text-cyan-200"><span className={`h-2.5 w-2.5 rounded-full ${isSpeaking ? 'animate-pulse bg-emerald-400' : 'bg-slate-500'}`} /> {isSpeaking ? 'Tutor speaking' : 'Tutor ready'}</span>
