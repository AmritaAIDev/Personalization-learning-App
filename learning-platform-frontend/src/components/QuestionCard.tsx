...
export default function QuestionCard({
  ...
  return (
    <section className={`rounded-[1.4rem] border border-[#ececf0] bg-white shadow-[0_12px_28px_rgba(20,20,30,0.05)] ${compact ? 'p-4 sm:p-5' : 'p-5 sm:p-7'}`}>
      <h2 className={`font-heading font-bold text-[#1a1a1f] ${compact ? 'text-lg leading-7 sm:text-xl sm:leading-8' : 'text-xl leading-8 sm:text-2xl'}`}>{questionText}</h2>
      <div className={compact ? 'mt-4 space-y-2.5' : 'mt-6 space-y-3'} role="radiogroup" aria-label="Answer options">
        {options.map((option) => {
