...
export default function Sidebar() {
  ...
  return (
    <aside className="fixed inset-x-0 bottom-0 z-40 flex h-[68px] w-full items-center border-t border-hairline bg-surface/85 px-2 backdrop-blur-xl md:sticky md:top-0 md:h-screen md:w-[76px] md:shrink-0 md:flex-col md:border-r md:border-t-0 md:bg-surface md:px-3 md:py-6 md:backdrop-blur-none lg:w-[248px] lg:px-4">
      <Link
        href="/"
        className="mb-9 hidden items-center gap-2.5 md:flex md:justify-center lg:justify-start lg:px-2"
      ...
      <nav
        className="flex min-w-0 flex-1 items-center justify-around gap-1 md:block md:w-full md:space-y-1"
          ...
          .map(({ label, href, icon: Icon }) => {
            ...
            return (
              <Link
                ...
                href={href}
                title={label}
                className={`group flex h-11 min-w-0 flex-1 flex-col items-center justify-center gap-0.5 rounded-lg px-2 text-[10px] font-medium transition-all duration-200 md:h-10 md:flex-row md:justify-center md:gap-3 md:text-[13.5px] lg:justify-start lg:px-3 ${
                  active
