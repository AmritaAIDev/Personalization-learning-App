...
export default function BloomRadar({ rows }: { rows: PerformanceRow[] }) {
  ...
  return (
    <section className="rounded-2xl border border-[#e8e1e3] bg-white p-5 shadow-[0_8px_22px_rgba(49,51,55,0.04)] sm:p-6">
      <h2 className="font-heading text-lg font-bold text-[#313337]">Cognitive skill profile</h2>
      <p className="mt-1 text-sm leading-6 text-[#6b6e75]">Skill balance across the attempt.</p>
      <div className="mx-auto mt-3 max-w-sm">
        <svg viewBox="0 0 200 200" role="img" aria-label="Radar chart of thinking skill performance">
          <polygon points={outer} fill="#fafafa" stroke="#e4e5e7" strokeWidth="1" />
          ...
          <polygon points={polygonPoints(rows, 1)} fill="rgba(227,21,64,0.18)" stroke="#e31540" strokeWidth="3" />
          {rows.map((row, index) => {
            const point = pointFor(index, rows.length, 64 * (row.score / 100));
            const label = pointFor(index, rows.length, 86);
            return (
              <g key={row.label}>
                <circle cx={point.x} cy={point.y} r="3.5" fill="#e31540" />
                <text x={label.x} y={label.y} textAnchor="middle" dominantBaseline="middle" className="fill-[#6b6e75] text-[9px] font-semibold">{row.label}</text>
              </g>
            );
          })}
        </svg>
      ...
      <dl className="mt-2 grid grid-cols-2 gap-2 text-xs sm:grid-cols-4">
        {rows.map((row) => <div key={row.label} className="rounded-lg bg-[#f7f4f5] px-2.5 py-2"><dt className="truncate text-[#6b6e75]">{row.label}</dt><dd className="mt-1 font-bold text-[#313337]">{row.score}%</dd></div>)}
      </dl>
    </section>
  );
}
