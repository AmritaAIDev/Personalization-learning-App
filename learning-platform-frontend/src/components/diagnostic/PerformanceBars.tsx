...
export default function PerformanceBars({
  ...
  return (
    <section className="rounded-2xl border border-[#e8e1e3] bg-white p-5 shadow-[0_8px_22px_rgba(49,51,55,0.04)] sm:p-6">
      <h2 className="font-heading text-lg font-bold text-[#313337]">{title}</h2>
      <p className="mt-1 text-sm leading-6 text-[#6b6e75]">{description}</p>
      <div className="mt-6 space-y-5">
        {rows.map((row) => (
          <div key={row.label}>
            <div className="mb-2 flex flex-wrap items-center justify-between gap-2 text-sm">
              <p className="font-semibold text-[#313337]">{row.label}</p>
