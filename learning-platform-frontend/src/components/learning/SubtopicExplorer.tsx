...
export default function SubtopicExplorer({ scope }: { scope: LearningScope }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const next = await apiFetch<ConceptBreakdown>(`/api/learning/subtopics?${params}`);
      setData(next);
  ...
  const active = data.items.find((item) => item.name === selected) ?? data.items[0];
  ...
  return (
    <section className="rounded-2xl bg-surface p-6 hairline elevate-sm">
      <div className="flex items-center gap-2">
        ...
        <span className="ml-auto text-xs text-ink-mute">{data.items.length} to explore</span>
      </div>
      ...
      <div key={active.name} className="mt-5 animate-fade rounded-xl bg-canvas p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <p className="text-sm font-semibold text-ink">{active.name}</p>
            <p className="mt-0.5 text-xs text-ink-mute">
              {active.questionCount} question{active.questionCount === 1 ? '' : 's'} · spans{' '}
              {[active.easy && 'Easy', active.medium && 'Medium', active.hard && 'Hard']
                .filter(Boolean)
        ...
        <div className="mt-4">
          ...
          <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-ink-mute">
            <span className="flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-full bg-primary/40" /> Easy {active.easy}
            </span>
            <span className="flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-full bg-primary/70" /> Medium {active.medium}
            </span>
            <span className="flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-full bg-primary" /> Hard {active.hard}
            </span>
          </div>
        </div>
      </div>
