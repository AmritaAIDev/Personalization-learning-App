      ...
      .catch((reason) =>
        setError(
          reason instanceof Error
            ? reason.message
            : "The topic could not be prepared.",
        ),
      )
  ...
  const startPlacement = async () => {
    ...
    try {
      ...
      setError(
        reason instanceof Error
          ? reason.message
          : "The placement check could not be started.",
      );
      setLaunching(false);
    }
  };
        ...
        {!loading && status?.needsDecision ? (
          <>
            ...
            <h1 className="mt-2 font-heading text-3xl font-semibold tracking-tight text-ink">
              How should we place you in {scope.topic}?
            </h1>
            <p className="mt-3 max-w-xl text-sm leading-6 text-ink-soft">
              Take a short placement check to begin at the level your answers
              support, or start from Level 1 and let adaptive practice build
              your route naturally.
            </p>
            {error ? (
              <p
            ...
            <div className="mt-7 grid gap-3 sm:grid-cols-2">
              ...
              >
                ...
                <span className="mt-2 block text-xs leading-5 text-white/65">
                  15 reviewed questions. Your result can place this topic up to
                  Level 9.
                </span>
              </button>
              <button
                ...
                className="rounded-2xl border border-hairline bg-canvas p-5 text-left transition hover:-translate-y-0.5 hover:border-primary/30 hover:bg-primary-tint"
              >
                <span className="text-sm font-semibold text-ink">
                  Start at Level 1
                </span>
                <span className="mt-2 block text-xs leading-5 text-ink-mute">
                  No assessment needed. Your first five-question set starts the
                  route.
                </span>
              </button>
            </div>
